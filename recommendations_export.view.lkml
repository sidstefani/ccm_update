view: recommendations_export {
  ...
  dimension: service {
    sql: CASE WHEN REGEXP_CONTAINS(${recommender},r'google.compute.') THEN 'Compute Engine'
          when REGEXP_CONTAINS(${recommender},r'google.cloudsql.') THEN 'Cloud SQL'
          when REGEXP_CONTAINS(${recommender},r'google.iam.') THEN 'IAM Policy'
          when REGEXP_CONTAINS(${recommender},r'google.run.') THEN 'Cloud Run Security'
          when REGEXP_CONTAINS(${recommender},r'google.resourcemanager.projectUtilization') THEN 'Project Utilization'
          when REGEXP_CONTAINS(${recommender},r'google.resourcemanager.serviceLimit') THEN 'Service Limit'
          ELSE '' END;;
  ...
  dimension: ticket_id  {
    type: string
    sql: REGEXP_EXTRACT(${name}, r'recommendations/([^/]+)') ;;
  }
  ...
  # dimension: pk {
  #   primary_key: yes
  #   sql: CONCAT(${recommendations_export__target_resources.target_resources},${recommender_subtype}) ;;
  # }
}
